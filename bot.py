...
TOKEN = "8680277831:AAGmavPv5Ctfc-sMz-O0H1lKmjAHKyPAikY"
ADMIN_ID = 1896036065  # ЗАМЕНИТЕ НА ВАШ ID
...
# ---------- РЕГИСТРАЦИЯ (С АВТОВЫДАЧЕЙ DOMINA ID) ----------
...
async def reg_gender(message: types.Message):
    ...
    await message.answer("Вы регистрируетесь как Домина или Пёс?", reply_markup=kb)
...
async def finish_registration(message: types.Message):
    ...
    # Автоматически выдаём Domina ID, если выбран статус Домина
    is_domina = (data.get("requested_role") == "domina")
    domina_id = None
    if is_domina:
        domina_id = generate_domina_id()
    
    cur.execute('''INSERT INTO users 
                  (user_id, username, name, gender, photo_file_id, age_verified, is_domina, domina_id, registered, domina_bio, dog_bio) 
                  VALUES (?,?,?,?,?,?,?,?,?,?,?)''',
                (user_id, username, data['name'], data['gender'], data['photo'], data['age_verified'],
                 is_domina, domina_id, 1, data.get('domina_bio', ''), data.get('dog_bio', '')))
    conn.commit()
    conn.close()
    
    if is_domina:
        await notify_new_user(user_id, is_domina=True)
        await message.answer(f"✅ Регистрация завершена! Вы Домина. Ваш DominaID: {domina_id}.\nТеперь вы видите анкеты псов, а псы видят вас.")
    else:
        await notify_new_user(user_id, is_domina=False)
        await message.answer("✅ Регистрация завершена! Вы Пёс.\nТеперь вы можете искать анкеты.")
...
async def make_domina(message: types.Message):
    ...
    if len(parts) < 2:
        await message.answer("/make_domina @username")
        return
    ...
    user_id, is_d = row
    if is_d:
        await message.answer("Уже Домина.")
        conn.close()
        return
    domina_id = generate_domina_id()
    ...
    conn.commit()
    conn.close()
    await message.answer(f"✅ {username} теперь Домина с ID {domina_id}.")
    await notify_new_user(user_id, is_domina=True)
    await bot.send_message(user_id, f"🎉 Вы стали Доминой! Ваш ID: {domina_id}")
...
async def remove_domina(message: types.Message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("/remove_domina @username")
        return
    username = parts[1].lstrip('@')
    conn = sqlite3.connect('dating.db')
    cur = conn.cursor()
...
async def list_dominas(message: types.Message):
    ...
    conn = sqlite3.connect('dating.db')
    cur = conn.cursor()
    cur.execute("SELECT username, name, domina_id FROM users WHERE is_domina=1")
    rows = cur.fetchall()
    conn.close()
    if not rows:
...
async def check_id(message: types.Message):
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("/checkid 123456")
        return
    try:
        did = int(parts[1])
    except:
    ...
    if row:
        ...
        await message.answer("Домина не найдена.")
...
async def broadcast(message: types.Message):
    ...
    if not text:
        ...
        return
    conn = sqlite3.connect('dating.db')
    users = conn.execute("SELECT user_id FROM users WHERE registered=1").fetchall()
    conn.close()
    if not users:
...
# Админ может написать любому пользователю
...
async def admin_message(message: types.Message):
    ...
    if len(parts) < 3:
        await message.answer("Использование: /admin_msg @username текст сообщения")
        return
    username = parts[1].lstrip('@')
    msg_text = parts[2]
    conn = sqlite3.connect('dating.db')
...
async def show_candidate(message: types.Message, candidates, index):
    ...
    buttons = []
    if viewer_is_domina and not is_domina:
...
async def like(call: types.CallbackQuery):
    ...
    if cur.fetchone():
        await call.answer("Вы уже лайкали.", show_alert=True)
        conn.close()
        return
    cur.execute("SELECT 1 FROM likes WHERE from_user=? AND to_user=?", (to_user, from_user))
    mutual = cur.fetchone()
    cur.execute("INSERT INTO likes (from_user, to_user, status) VALUES (?,?,'pending')", (from_user, to_user))
    conn.commit()
    if mutual:
...
# ---------- ОТПРАВКА СООБЩЕНИЙ ----------
@dp.message(F.text, lambda msg: msg.from_user.id in active_chats and not msg.text.startswith('/'))
async def send_message(message: types.Message):
    ...
    if not cur.fetchone():
        ...
        row = cur.fetchone()
        if not row or not row[0]:
            await message.answer("❌ Нет взаимной симпатии.")
            conn.close()
            return
...
async def reply_to_message(call: types.CallbackQuery):
    ...
    to_user = int(call.data.split("_")[1])
    active_chats[from_user] = to_user
    await call.message.answer("✅ Ответьте сообщением.\n/endchat - закрыть")
    await call.answer()
...
async def reset_dislikes(message: types.Message):
    ...
    conn.commit()
    conn.close()
    await message.answer("✅ Список дизлайков очищен.")

# ---------- СБРОС АНКЕТЫ (УДАЛЕНИЕ ВСЕХ ДАННЫХ) ----------
...
async def reset_profile(message: types.Message):
    ...
    await message.answer("⚠️ *Вы уверены?* Все данные вашей анкеты, лайки, дизлайки, сообщения будут безвозвратно удалены.\nВы сможете зарегистрироваться заново.", parse_mode="Markdown", reply_markup=kb)
...
async def confirm_reset(call: types.CallbackQuery):
    ...
    cur.execute("DELETE FROM dislikes WHERE from_user=? OR to_user=?", (user_id, user_id))
    ...
    temp_candidates.pop(user_id, None)
    edit_data.pop(user_id, None)
    await call.message.edit_text("🗑 Ваша анкета полностью удалена. Вы можете начать заново с /start")
    await call.answer()

@dp.callback_query(F.data == "cancel_reset")
async def cancel_reset(call: types.CallbackQuery):
    await call.message.edit_text("❌ Сброс отменён.")
    await call.answer()
