...
ADMIN_USER_ID = 8117530336  # ЗАМЕНИТЕ НА ВАШ ID (число)

# ID кастомных эмодзи (получить через @getidsbot, если есть Premium)
# Если не заданы — кнопки будут без иконок
DOMINA_EMOJI_ID = 5332470445109172776      # например "1234567890123456789"
DOG_EMOJI_ID = 5332508880771503602
SUCCESS_EMOJI_ID = 5332807252149557358
DANGER_EMOJI_ID = 5332300901275161454
...
# ---------- СОСТОЯНИЯ РЕГИСТРАЦИИ И РЕДАКТИРОВАНИЯ ----------
class RegState(StatesGroup):
    age_confirm = State()
    name = State()
    photo = State()
    gender = State()
    bio_type = State()    # 'domina' или 'dog'
    domina_bio = State()
    dog_bio = State()
...
# ---------- СТАРТ И РЕГИСТРАЦИЯ (С ПРИВЕТСТВИЕМ ОТ АДМИНА) ----------
...
async def reg_gender(message: types.Message, state: FSMContext):
    ...
    # Спросим, кто он: Домина или Пёс (но фактически роль позже выдаст админ, но для анкеты запросим разные поля)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👠 Домина", callback_data="reg_role_domina")],
        [InlineKeyboardButton(text="🦴 Пёс", callback_data="reg_role_dog")]
    ])
    await message.answer("Вы регистрируетесь как Домина или Пёс?\n(Статус Домины позже подтвердит администратор)", reply_markup=kb)
    await state.set_state(RegState.bio_type)
    await message.answer("Ваш пол сохранён.", reply_markup=ReplyKeyboardRemove())

@dp.callback_query(RegState.bio_type, F.data.in_(["reg_role_domina", "reg_role_dog"]))
async def reg_role(callback: types.CallbackQuery, state: FSMContext):
    role = callback.data.split("_")[2]  # 'domina' или 'dog'
    ...
    if role == "domina":
        await callback.message.edit_text(
            ...
            "(Можете использовать эмодзи и кастомные эмодзи)"
        )
...
async def finish_registration(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect('dating.db')
    cur = conn.cursor()
    # Сохраняем оба поля bio, но отображать будем в зависимости от роли
    ...
    conn.close()
    await message.answer(
        "✅ Регистрация завершена! Вы зарегистрированы как Пёс.\n"
...
async def search(message: types.Message):
    ...
    # Домина видит всех псов; Пёс видит только верифицированных домин (is_domina=1)
    need_role = 0 if my_role == 1 else 1
    if my_role == 1:
        # Домина ищет псов
        condition = "is_domina = 0"
    else:
        # Пёс ищет только Домин
...
async def show_candidate(message: types.Message, candidates, index):
    ...
    if is_domina:
        ...
        role_text = "🦴 Пёс"
        if not domina_id:
            role_text += "\n⚠️ Непроверенный аккаунт (возможен скам)"
    text = f"👤 {name}\n{role_text}\n\n📝 {bio_text}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
