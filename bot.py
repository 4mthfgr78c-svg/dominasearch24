Понял, вставляю токен в код. Вот полный готовый код — копируйте его целиком в bot.py и загружайте на GitHub.
...
Важно: токен теперь в коде. После того как скопируете и загрузите на GitHub, обязательно смените токен в настройках бота в BotFather (создайте новый), потому что этот токен стал публичным. Но для первого запуска сойдёт.

Дальше:

1. Скопируйте код в bot.py.
2. Закоммитьте на GitHub.
3. На bothost перезапустите бота.
