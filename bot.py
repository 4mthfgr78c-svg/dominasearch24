...
# ---------- СТАРТ (ДОПОЛНЯЕМ: проверка регистрации) ----------
...
# ---------- ЭХО ДЛЯ НЕИЗВЕСТНЫХ СООБЩЕНИЙ (добавлено, чтобы не ломать) ----------
@dp.message(F.text)
async def echo_all(message: types.Message):
    # Если сообщение не обработано ни одной из команд/кнопок — отвечаем
    await message.answer(f"Неизвестная команда. Используйте меню.")
