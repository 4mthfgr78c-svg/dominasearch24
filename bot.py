...
async def role_callback(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    role = callback.data.split("_")[1]  # 'domina' или 'dog'
...
async def show_candidate(message: types.Message, candidates, index):
    ...
    # Определяем, кто смотрит (чтобы добавить кнопку "Написать" для Домины)
    ...
    buttons = []
    if viewer_is_domina and not is_domina:   # Домина смотрит на пса
...
async def like(call: types.CallbackQuery):
    ...
    conn.close()
    await call.message.delete()
    # Обновляем поиск, чтобы убрать пролайканную анкету
...
async def write_first(call: types.CallbackQuery):
    from_user = call.from_user.id
    to_user = int(call.data.split("_")[1])
    # Проверяем, что отправитель — Домина
    conn = sqlite3.connect('dating.db')
    cur = conn.cursor()
...
async def open_chat(call: types.CallbackQuery):
    ...
    await call.message.answer("✅ Диалог открыт. Напишите сообщение собеседнику.\nЧтобы закончить, нажмите /endchat")
    await call.answer()
...
async def show_messages(message: types.Message):
    ...
    for from_u, to_u, msg_text, ts in reversed(rows):
        ...
        # Пытаемся получить username собеседника
